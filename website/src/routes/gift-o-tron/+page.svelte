        ...
        <div class="min-w-0 flex-1">
            <form method="POST" class="relative" use:enhance={({ form, data, action, cancel }) => {
                ...
                <div class="bg-slate-50 overflow-hidden rounded-lg border border-gray-300 shadow-sm focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 p-4">
                    ...
                    <button type="submit" class="float-right inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Generate Ideas</button>
                </div>
            </form>
        </div>
    </div>
